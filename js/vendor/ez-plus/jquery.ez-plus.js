...
(function ($, window, document, undefined) {
    var EZP = {
        ...
        startZoom: function () {
			...
			if(self.$elem.closest('.product-slider-wrapper').length){
				self.nzOffsetLeft = self.nzOffset.left - $('.product-slider-wrapper').offset().left;		
			}
			else self.nzOffsetLeft =  self.nzOffset.left;
            ...
            if (self.options.touchEnabled) {
				...
				self.zoomContainer.bind('dbltap.ezpspace', function (e) {
						if (self.options.zoomType === 'inner') {
				...
				self.zoomContainer.bind('touchmove.ezpspace', function (e) {
						if(self.isWindowActive === true || self.isLensActive === true){
							if (self.options.zoomType === 'inner' && self.isWindowActive === true) {
								self.showHideWindow('show');
							}
							e.preventDefault();
							var touch = e.originalEvent.touches[0] || e.originalEvent.changedTouches[0];
        ...
        setElements: function (type) {
            ...
            if (type === 'show') {
                if (self.isWindowSet) {
                    ...
                    if (self.options.showLens && !$('body').hasClass('touch')) {
                        self.showHideLens('show');
                    }
                    if (self.options.tint && self.options.zoomType !== 'inner') {
    ...
    $.fn.ezPlus.options = {
        attrImageZoomSrc: 'data-zoom-image', // attribute to plugin use for zoom
        borderColour: '#888',
        borderSize: 4,
